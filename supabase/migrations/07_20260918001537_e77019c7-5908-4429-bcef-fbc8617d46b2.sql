...
REVOKE ALL ON FUNCTION public.protect_activity_assignment_fields() FROM anon, authenticated, service_role, PUBLIC;
DROP FUNCTION public.protect_activity_assignment_fields();

CREATE OR REPLACE FUNCTION private.protect_activity_assignment_fields()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION private.protect_activity_assignment_fields() FROM anon, authenticated, service_role, PUBLIC;

CREATE TRIGGER activities_protect_assignment_fields
BEFORE UPDATE ON public.activities
FOR EACH ROW
EXECUTE FUNCTION private.protect_activity_assignment_fields();